...
Provides 18 tools to read, write, create, and manage Google Docs and Drive files.
Licensed under MIT.
"""
...
def tool_authenticate():
    try:
        ...
        if creds and creds.valid:
            return _ok(f"\u2705 Successfully authenticated with Google!\nToken saved to: {TOKEN_FILE}")
...
def tool_replace_text(doc_id_or_url, find, replace_with):
    try:
        doc_id = extract_doc_id(doc_id_or_url)
        service = docs_service()
        requests = [{"replaceAllText": {"containsText": {"text": find, "matchCase": True}, "replaceText": replace_with}}]
...
def tool_set_heading(doc_id_or_url, start_index, end_index, heading_level):
    ...
    if heading_level not in valid:
        return _err(f"Invalid heading_level '{heading_level}'.")
    try:
        doc_id = extract_doc_id(doc_id_or_url)
        service = docs_service()
...
def tool_list_documents(query="", max_results=20):
    try:
        ...
        lines = ["\U0001f4c1 **Google Docs**\n"]
        for f in files:
            lines.append(f"- **{f['name']}**\n  \U0001f194 `{f['id']}`\n  \U0001f552 {f.get('modifiedTime', '')}\n  \U0001f517 {f.get('webViewLink', '')}\n")
        return _ok("\n".join(lines))
    except HttpError as e:
        return _err(f"Google API error: {e}")
    except Exception as e:
...
def tool_copy_document(doc_id_or_url, new_title):
    try:
        ...
        return _ok(f"\u2705 Copied to: **{new_title}**\n\U0001f194 `{new_id}`\n\U0001f517 {url}")
    except HttpError as e:
        return _err(f"Google API error: {e}")
    except Exception as e:
        return _err(str(e))
...
def tool_delete_document(doc_id_or_url):
    try:
        ...
        return _ok(f"\U0001f5d1\ufe0f Document `{doc_id}` moved to trash.")
    except HttpError as e:
        return _err(f"Google API error: {e}")
    except Exception as e:
        return _err(str(e))
...
def tool_rename_document(doc_id_or_url, new_title):
    try:
        ...
        return _ok(f"\u2705 Renamed `{doc_id}` to **{new_title}**.")
    except HttpError as e:
        return _err(f"Google API error: {e}")
    except Exception as e:
        return _err(str(e))
...
def tool_share_document(doc_id_or_url, email, role="writer"):
    ...
    try:
        ...
        return _ok(f"\u2705 Shared `{doc_id}` with {email} as **{role}**.")
    except HttpError as e:
        return _err(f"Google API error: {e}")
    except Exception as e:
        return _err(str(e))
...
def tool_export_document(doc_id_or_url, export_format="text/plain"):
    ...
    if export_format not in valid:
        return _err(f"Invalid format '{export_format}'.")
    try:
        doc_id = extract_doc_id(doc_id_or_url)
        service = drive_service()
...
TOOLS = [
    ...
    Tool(name="google_docs_authenticate", description="Authenticate with Google via OAuth 2.0. Opens a browser window. Requires credentials.json at ~/.config/goose/google-docs-extension/credentials.json", inputSchema={"type": "object", "properties": {}, "required": []}),
    Tool(name="google_docs_read", description="Read the full text content of a Google Doc.", inputSchema={"type": "object", "properties": {"doc_id_or_url": {"type": "string"}}, "required": ["doc_id_or_url"]}),
    Tool(name="google_docs_get_metadata", description="Get metadata (title, ID, revision) for a Google Doc.", inputSchema={"type": "object", "properties": {"doc_id_or_url": {"type": "string"}}, "required": ["doc_id_or_url"]}),
    Tool(name="google_docs_create", description="Create a new Google Doc with optional initial text.", inputSchema={"type": "object", "properties": {"title": {"type": "string"}, "body_text": {"type": "string", "default": ""}}, "required": ["title"]}),
    ...
    Tool(name="google_docs_insert_text", description="Insert text at a specific character index.", inputSchema={"type": "object", "properties": {"doc_id_or_url": {"type": "string"}, "text": {"type": "string"}, "index": {"type": "integer"}}, "required": ["doc_id_or_url", "text", "index"]}),
    Tool(name="google_docs_delete_range", description="Delete a character range in a Google Doc.", inputSchema={"type": "object", "properties": {"doc_id_or_url": {"type": "string"}, "start_index": {"type": "integer"}, "end_index": {"type": "integer"}}, "required": ["doc_id_or_url", "start_index", "end_index"]}),
    Tool(name="google_docs_apply_bold", description="Apply bold formatting to a range.", inputSchema={"type": "object", "properties": {"doc_id_or_url": {"type": "string"}, "start_index": {"type": "integer"}, "end_index": {"type": "integer"}}, "required": ["doc_id_or_url", "start_index", "end_index"]}),
    Tool(name="google_docs_set_heading", description="Set heading style (HEADING_1-6 or NORMAL_TEXT) for a range.", inputSchema={"type": "object", "properties": {"doc_id_or_url": {"type": "string"}, "start_index": {"type": "integer"}, "end_index": {"type": "integer"}, "heading_level": {"type": "string", "enum": ["HEADING_1","HEADING_2","HEADING_3","HEADING_4","HEADING_5","HEADING_6","NORMAL_TEXT"]}}, "required": ["doc_id_or_url", "start_index", "end_index", "heading_level"]}),
    Tool(name="google_docs_batch_update", description="Send a raw Docs API batchUpdate for advanced edits.", inputSchema={"type": "object", "properties": {"doc_id_or_url": {"type": "string"}, "requests_json": {"type": "string"}}, "required": ["doc_id_or_url", "requests_json"]}),
    ...
    Tool(name="google_docs_copy", description="Create a copy of a Google Doc.", inputSchema={"type": "object", "properties": {"doc_id_or_url": {"type": "string"}, "new_title": {"type": "string"}}, "required": ["doc_id_or_url", "new_title"]}),
