...
def tool_authenticate() -> CallToolResult:
    try:
...
def tool_read_document(doc_id_or_url: str) -> CallToolResult:
    try:
...
def tool_get_document_metadata(doc_id_or_url: str) -> CallToolResult:
    try:
        doc_id = extract_doc_id(doc_id_or_url)
        service = docs_service()
        doc = service.documents().get(documentId=doc_id).execute()
...
def tool_create_document(title: str, body_text: str = "") -> CallToolResult:
    try:
        service = docs_service()
...
def tool_append_text(doc_id_or_url: str, text: str) -> CallToolResult:
    try:
        doc_id = extract_doc_id(doc_id_or_url)
        service = docs_service()
        doc = service.documents().get(documentId=doc_id).execute()
...
def tool_replace_text(doc_id_or_url: str, find: str, replace_with: str) -> CallToolResult:
    try:
        doc_id = extract_doc_id(doc_id_or_url)
        service = docs_service()
        requests = [{
...
def tool_insert_text_at_index(doc_id_or_url: str, text: str, index: int) -> CallToolResult:
    try:
        doc_id = extract_doc_id(doc_id_or_url)
        service = docs_service()
...
def tool_delete_text_range(doc_id_or_url: str, start_index: int, end_index: int) -> CallToolResult:
    try:
        doc_id = extract_doc_id(doc_id_or_url)
        service = docs_service()
...
def tool_apply_bold(doc_id_or_url: str, start_index: int, end_index: int) -> CallToolResult:
    try:
        doc_id = extract_doc_id(doc_id_or_url)
        service = docs_service()
...
def tool_set_heading(doc_id_or_url: str, start_index: int, end_index: int, heading_level: str) -> CallToolResult:
...
def tool_batch_update(doc_id_or_url: str, requests_json: str) -> CallToolResult:
    try:
        doc_id = extract_doc_id(doc_id_or_url)
...
def tool_list_documents(query: str = "", max_results: int = 20) -> CallToolResult:
    try:
...
def tool_copy_document(doc_id_or_url: str, new_title: str) -> CallToolResult:
    try:
        doc_id = extract_doc_id(doc_id_or_url)
        service = drive_service()
...
def tool_delete_document(doc_id_or_url: str) -> CallToolResult:
    try:
        doc_id = extract_doc_id(doc_id_or_url)
        service = drive_service()
...
def tool_rename_document(doc_id_or_url: str, new_title: str) -> CallToolResult:
    try:
        doc_id = extract_doc_id(doc_id_or_url)
        service = drive_service()
...
def tool_share_document(doc_id_or_url: str, email: str, role: str = "writer") -> CallToolResult:
...
def tool_export_document(doc_id_or_url: str, export_format: str = "text/plain") -> CallToolResult:
